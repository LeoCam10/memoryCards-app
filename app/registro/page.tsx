...
export default function JuegoPage() {
  ...
  const finalizarPartida = async (motivo: MotivoFin) => {
    ...
    if (motivo === "abandono") {
      ...
      mensaje = `${turnoActual} abandonó la partida. ${ganador} gana automáticamente.`;
    }
  ...
  return (
    <main className="page">
      <div className="container">
        {configuracion && (
          <div className="panel">
            <p><strong>Número de partida:</strong> <span className="highlight">#{numeroPartidaActual}</span></p>
            <p><strong>Dificultad:</strong> {configuracion.dificultad}</p>
            <p><strong>Tiempo configurado:</strong> {configuracion.tiempo === null ? "Sin tiempo máximo" : `${configuracion.tiempo} min`}</p>
            <p><strong>Tipo de cartas:</strong> {configuracion.tipoCartas}</p>
            <p><strong>Cantidad de cartas:</strong> {cartas.length}</p>
            <p><strong>Intentos máximos:</strong> {maximoIntentos}</p>
            <p><strong>Intentos totales:</strong> {intentosTotales}</p>
            <p>
        ...
        <div className="panel">
          <div className="game-status">
            <div className="player-card">
              <h2>{jugador1?.nombre_usuario ?? "Jugador 1"}</h2>
            ...
            <div className="turn-box">
              ...
              <div className="info-box" style={{ width: "140px", margin: "10px auto" }}>
                <p>Tiempo</p>
                <strong>{formatearTiempo(tiempoRestante)}</strong>
              </div>

              <button className="btn btn-danger" onClick={() => finalizarPartida("abandono")}>
                Abandonar Partida
              </button>
            </div>

            <div className="player-card">
              <h2>{jugador2?.nombre_usuario ?? "Jugador 2"}</h2>
